...
canvas.addEventListener('mousemove', function(evt) {
   ...
   for ( var i = 0; i < length; i++) {
      if (  mousePos.x > data[i].coordinates.x && 
            ...
            document.getElementById("img").setAttribute("src", "faces/" +  data[i].image);
            document.getElementById("id").innerHTML = "ID: " + data[i].id;
            document.getElementById("firstname").innerHTML = "Naam: " + data[i].firstname;
            document.getElementById("numberofcontacts").innerHTML = "Aaantal bedrijfven: " + data[i].numberofcontacts;
            document.getElementById("stage").innerHTML = "heeft deze student een stage: " + data[i].heeftstage;
...
function getData () {
   xmlHttp.open("GET", "bpv_st_view_data.php", true);
   xmlHttp.send();
}